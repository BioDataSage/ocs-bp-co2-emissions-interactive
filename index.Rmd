...
```{r, eval=FALSE}
CO2_emissions <- readxl::read_xlsx(here("www", "data","raw", "yearly_co2_emissions_1000_tonnes.xlsx"))
gdp_growth    <- readxl::read_xlsx(here("www", "data", "raw", "gdp_per_capita_yearly_growth.xlsx"))
energy_use    <- readxl::read_xlsx(here("www", "data", "raw", "energy_use_per_person.xlsx"))
```
...
```{r, eval=FALSE}
us_disaster <- readr::read_csv(here("www", "data", "raw", "disasters.csv"), skip = 2)
```
...
```{r, eval=FALSE}
us_temperature <- readr::read_csv(here("www", "data", "raw", "temperature.csv"), skip = 4, na = "-99")
```
...
save(CO2_emissions, 
     ...
     us_disaster, 
     us_temperature, 
     file = here::here("www", "data", "imported", "co2_data_imported.rda"))
```
...
```{r, eval=FALSE}
load(here::here("www", "data", "imported", "co2_data_imported.rda"))
```
...
We will use the `%>%` pipe from the `dplyr` package, which can be used to define the input for later sequential steps. 
This allows us to use our `CO2_emissions` data and modify it sequentially using  multiple steps. We will start with just one step however, but we will see the utility of this soon where we will have multiple steps each having a %>% at the end into the next step. We can think of this as a "take this data and then" statement". 

The `<-` assignment operator can as usual be used to reassign the object to the modified version after all the steps after the `%>%` are completed. 
...
Note that here we are now using multiple steps with our `%>%` pipe operator. So the CO2_emissions data is used to rename Contry to country and then the out put of this is taken to be mutated to have Year as a numeric variable.
...
gdp_growth %>% # let's check before we reassign 
  pivot_longer(cols = -country,
               names_to = "Year",
...
```{r DW_Code4-hint-1}
energy_use %>% # let's check before we reassign
  pivot_longer(cols = -country,
               names_to = "Year",
...
```{r, eval = FALSE}
save(data_long, file = here::here("www", "data", "wrangled", "wrangled_data.rda"))
readr::write_csv(data_long, path = here::here("www", "data","wrangled", "wrangled_data.csv"))
```
...
quiz(caption = "",
  ...
  question("Which one of the pipe operators (from the `dplyr` package) should be used right after a variable name if we want to perform a sequence of operations on that variable?",
    answer("`%>%`", correct = TRUE),
    answer("`%<%`", message = "This is not a valid pipe operator."),
    answer("`%T>%`", message = "This is the side-effects operator."),
    answer("`>`", correct = "This is not a valid pipe operator"),
    allow_retry = TRUE,
    random_answer_order = TRUE
  ),
...
```{r DW_Exercise1-check}
grade_this_code(
  correct = "Good work!",
  incorrect = "Not quite. {random_encouragement()}"
)
```
...
```{r, eval = FALSE}
save(wide_US, file = here::here("www", "data", "wrangled", "wrangled_US_data.rda"))
readr::write_csv(wide_US, path = here::here("www", "data", "wrangled", "wrangled_US_data.csv"))
```
...
```{r, eval=FALSE}
load(here::here("www", "data", "wrangled", "wrangled_US_data.rda"))
```
...
[autocorrelation](https://en.wikipedia.org/wiki/Autocorrelation){target="_blank"}  
